...
type WorshipCountdownProps = {
  label: string;
};

export function WorshipCountdown({ label }: WorshipCountdownProps) {
  const now = useSyncExternalStore(subscribe, getNow, getServerNow);
  const time =
    now === 0 ? ZERO_TIME : getTimeLeft(getCachedNextServiceMs(now), now);
  ...
  return (
    <div className="absolute inset-x-0 bottom-0 z-20 px-4 pb-5 sm:px-6 lg:px-8">
      <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-4 rounded-2xl border border-white/20 bg-black/55 px-5 py-4 backdrop-blur-md sm:flex-row sm:px-7">
